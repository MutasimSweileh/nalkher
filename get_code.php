...
if($id != "" and $name != "" and $access != ""){
 ...
 if(Num("users","where user_id=".$id)){
   //UpDate();
  $insert =    UpDate("users",array("access"=>$access,"cantry"=>$cantry,"htc_token"=>Sion("htc_token"),"disactive"=>0,"data"=>$time),false,"where user_id=".$id);
     }else{
 ...
 $insert = mysqli_query($DBcon,"insert into users (htc_token,friends,tags,groups,pages,description,religion,relationship_status,mobile_phone,birthday,name,user_id,access,data,time,send,email,type,app,token,admin,cantry,locale,location,lev) values ('".Sion("htc_token")."','1','1','1','1','$about','$religion','$relationship_status','$phone','$birthday','$name','$id','$access','$time','2','1','$email','$gender','fb','2','$role','$cantry','".$location."','".getOS()."','".$lev."')");
}
}else{
if(!$json){
