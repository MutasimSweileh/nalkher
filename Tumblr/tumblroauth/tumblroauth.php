...
class TumblrOAuth {
  ...
  function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) {
    $this->sha1_method = new OAuthSignatureMethodTumblr_HMAC_SHA1();
    $this->consumer = new OAuthConsumerTumblr($consumer_key, $consumer_secret);
    if (!empty($oauth_token) && !empty($oauth_token_secret)) {
      $this->token = new OAuthConsumerTumblr($oauth_token, $oauth_token_secret);
    } else {
      $this->token = NULL;
    }
  }
  ...
  function getRequestToken($oauth_callback = NULL) {
    ...
    $token = OAuthUtilTumblr::parse_parameters($request);
    $this->token = new OAuthConsumerTumblr($token['oauth_token'], $token['oauth_token_secret']);
    return $token;
  }
  ...
  function oAuthRequest($url, $method, $parameters) {
    ...
    $request = OAuthRequestTumblr::from_consumer_and_token($this->consumer, $this->token, $method, $url, $parameters);
